...
### 5.1 Certification thresholds (v1 — to recalibrate after 6 months of data)

| Level | Min runs | Min avg_rating | Max disable_rate | Extra gate |
|-------|----------|----------------|-------------------|------------|
| Bronze | 10 | 4.0 | 0.20 | — |
| Silver | 50 | 4.3 | 0.12 | — |
| Gold | 200 | 4.5 | 0.08 | Human review passed |

A skill that drops below its current tier's floor on rolling 30-day metrics is downgraded automatically. The Matching Agent weights certification level as one signal among several (see `docs/tech/03-matching-agent.md` when written).
...
- **Author attribution & claim-based payout**: `author.display_name` reflects the original author; Symbiose keeps 100 % of the author share until the author formally claims the skill (via domain verification, OAuth on the source platform, or a manual review). On successful claim, Symbiose pays out the author's share **retroactive to the 90 days preceding the claim date** — earlier usage is not reimbursed. This creates an incentive to claim without opening the door to phantom transfers.
...
The following 10 decisions are **locked** in v1. They shape the examples and field definitions above.
...
### 9.8 Token pricing — static at MVP, dynamic in Phase 2

`economics.price_tokens` is a **static** value set by the author (or Symbiose for natives) at publication time. No runtime price adjustment in v1.

Rationale: predictability for authors (they know what they earn per run) > revenue optimisation at MVP. Dynamic pricing (demand / scarcity / budget-fit) is deferred to Phase 2 once usage data exists to calibrate pricing signals.

### 9.9 Revenue split for imported unclaimed skills — retroactive 90-day payout on claim

See §6.3. Symbiose keeps 100 % of the author share until the author formally claims the skill (domain verification / OAuth / manual review). On claim, Symbiose pays retroactively for the **90 days preceding the claim date**. Earlier usage is not reimbursed.

Rationale: incentivises legitimate authors to claim quickly without creating an escrow liability for every imported skill — and without sending phantom payouts to accounts nobody controls.

### 9.10 Certification thresholds — v1 draft locked, recalibrate at 6 months

See §5.1 table. Bronze / Silver / Gold thresholds are fixed for v1. Auto-downgrade on 30-day rolling metrics. Thresholds will be reviewed after 6 months of real data.

Rationale: starting strict protects the "certified" label. Data-driven recalibration prevents the thresholds from ossifying against reality.

## 10. Next steps

1. Freeze the JSON-Schema derived from this document → `docs/tech/schemas/skill.schema.json`.
2. Write **Matching Agent spec v1** (`docs/tech/03-matching-agent.md`) grounded on this manifest.
3. Define the **first 5 native skills** to build, using this manifest as the template.
